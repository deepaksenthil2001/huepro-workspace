...
import { Plus, Search, Edit2, Trash2, X, Package, Layers, Hash } from 'lucide-react';
...
export default function Materials() {
  ...
  const [formData, setFormData] = useState({ name: '', category: 'Paint', quantity: '', unit: 'Liters', status: 'In Stock' });
  ...
  const openModal = (material = null) => {
    if (material) {
      ...
      setFormData({ name: '', category: 'Paint', quantity: '', unit: 'Liters', status: 'In Stock' });
    }
    setIsModalOpen(true);
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-8 rounded-[2rem] border border-neutral-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)]">
        <div>
          <h2 className="text-3xl font-extrabold text-neutral-900 tracking-tight">Paint & Stock</h2>
          <p className="text-neutral-500 mt-1.5 font-medium">Manage your inventory, paints, and tools.</p>
        </div>
        <div className="flex gap-4 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400" />
            <input type="text" placeholder="Search inventory..." className="w-full bg-neutral-50 border border-neutral-200 rounded-full pl-11 pr-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all" />
          </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-neutral-100 bg-neutral-50/50">
                <th className="p-6 text-xs font-bold text-neutral-400 uppercase tracking-wider">Item Details</th>
                <th className="p-6 text-xs font-bold text-neutral-400 uppercase tracking-wider">Quantity</th>
            ...
            <tbody className="divide-y divide-neutral-100">
              {isLoading ? (
                ...
                <tr><td colSpan="4" className="p-10 text-center text-neutral-500 font-medium">No items found in stock!</td></tr>
              ) : (
                materials.map((item) => (
                  <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-neutral-50/80 transition-colors group">
                    <td className="p-6">
                    ...
                    <td className="p-6">
                      <div className="flex items-center gap-2 text-sm font-bold text-neutral-800">
                        ...
                        {item.quantity} <span className="text-xs font-medium text-neutral-500">{item.unit}</span>
                      </div>
                    </td>
                    <td className="p-6">
                    ...
                    <td className="p-6">
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => openModal(item)} className="p-2.5 bg-white hover:bg-neutral-100 rounded-xl text-neutral-500 hover:text-indigo-600 transition-colors border border-neutral-200 shadow-sm"><Edit2 className="w-4 h-4" /></button>
                        <button onClick={() => handleDelete(item.id)} className="p-2.5 bg-white hover:bg-red-50 rounded-xl text-neutral-500 hover:text-red-500 transition-colors border border-neutral-200 shadow-sm"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </motion.tr>
                ))
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-neutral-900/40 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="bg-white rounded-[2.5rem] p-8 md:p-10 w-full max-w-xl shadow-[0_20px_60px_rgba(0,0,0,0.15)] relative overflow-hidden">
              ...
              <div className="flex justify-between items-center mb-8 mt-2">
                <div>
                  <h3 className="text-2xl font-extrabold text-neutral-900">{editingMaterial ? 'Update Item' : 'Add New Item'}</h3>
                  <p className="text-neutral-500 text-sm mt-1 font-medium">Enter stock details.</p>
                </div>
              ...
              <form onSubmit={handleSave} className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="col-span-1 md:col-span-2">
                    <label className="block text-xs font-bold text-neutral-500 mb-1.5 ml-1 uppercase tracking-wide">Item Name</label>
                    <input required type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full bg-neutral-50 border border-neutral-200 rounded-2xl px-5 py-3.5 text-neutral-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all font-medium" placeholder="e.g. Asian Paints Royale" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-neutral-500 mb-1.5 ml-1 uppercase tracking-wide">Category</label>
                    <select value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})} className="w-full bg-neutral-50 border border-neutral-200 rounded-2xl px-5 py-3.5 text-neutral-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all font-medium appearance-none">
                      <option>Paint</option><option>Primer</option><option>Tools</option><option>Consumables</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-neutral-500 mb-1.5 ml-1 uppercase tracking-wide">Quantity</label>
                    <input required type="number" value={formData.quantity} onChange={(e) => setFormData({...formData, quantity: e.target.value})} className="w-full bg-neutral-50 border border-neutral-200 rounded-2xl px-5 py-3.5 text-neutral-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all font-medium" placeholder="e.g. 50" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-neutral-500 mb-1.5 ml-1 uppercase tracking-wide">Unit</label>
                    <select value={formData.unit} onChange={(e) => setFormData({...formData, unit: e.target.value})} className="w-full bg-neutral-50 border border-neutral-200 rounded-2xl px-5 py-3.5 text-neutral-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all font-medium appearance-none">
                      <option>Liters</option><option>Kgs</option><option>Pieces</option><option>Boxes</option>
                    </select>
                  </div>
                  <div>
                ...
                <div className="flex gap-4 mt-10 pt-6 border-t border-neutral-100">
                  ...
                  <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }} type="submit" className="w-2/3 py-4 rounded-2xl text-white font-bold shadow-lg" style={{ background: premiumGradient }}>
                    {editingMaterial ? 'Save Changes' : 'Add Item'}
                  </motion.button>
                </div>
              </form>
            </motion.div>
